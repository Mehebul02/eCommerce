...
const MobileNavigation = () => {
  ...
  useEffect(() => {
    // ব্রাউজার রেন্ডার হওয়ার পরেই true হবে
    setMounted(true);
  }, []);

  if (!mounted) return null; // SSR এ কিছুই রেন্ডার হবে না
    ...
    <>
      ...
      >
        <div className="fixed inset-0 z-10 flex items-start justify-center p-4 bg-black/90 w-screen overflow-y-auto top-0">
          <DialogPanel className="w-full max-w-md rounded-xl bg-black border border-indigo-300 p-4 backdrop-blur-2xl transition duration-300 ease-out">
            {/* Header */}
